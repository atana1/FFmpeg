...
static const AVOption peakpoints2_options[] = {
    ...
    { "mode", "do lookup for the song or store the song", OFFSET(mode), AV_OPT_TYPE_INT, {.i64=1}, 0, 1, FLAGS, "mode"},
...
static int filter_frame(AVFilterLink *inlink, AVFrame *samples)
{
    ...
    for (i = 0; i < nb_samples; i++) {
        ...
        if (p->index == p->windowSize) {
            ...
            ppointsStats(ctx, p);
            //p->time = p->time + 1;
            memmove(p->data, p->data + (p->windowSize * 1)/4, sizeof(*p->data)*((p->windowSize*3)/4));
            p->index = (p->windowSize * 3)/4;
            p->buffFlag = 1;
        }
    }
